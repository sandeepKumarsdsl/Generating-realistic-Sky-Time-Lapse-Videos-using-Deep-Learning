...
def create_logger(logging_dir):
    ...
    """
    #if dist.get_rank() == 0:  # real logger # 6 - Fix for Single GPU
...
def create_tensorboard(tensorboard_dir):
    ...
    """
    #if dist.get_rank() == 0:  # real tensorboard  # 4 - Fix for Single GPU
    if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
...
def write_tensorboard(writer, *args):
    ...
    '''
    #if dist.get_rank() == 0:  # real tensorboard # 5 - Fix for Single GPU
    if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
...
def update_ema(ema_model, model, decay=0.9999):
    ...
    for name, param in model_params.items():
        ...
        if name in ema_params: #Fix for EMA update
...
def setup_distributed(backend="nccl", port=29500):
    """Initialize distributed training environment (single-GPU safe)."""

    num_gpus = torch.cuda.device_count()

    # SINGLE GPU: Skip distributed setup
    ''' # 1 - Fix for Single GPU
    if num_gpus == 1:
        print("Single GPU detected. Skipping distributed setup.")
        os.environ["LOCAL_RANK"] = "0"
        os.environ["RANK"] = "0"
        os.environ["WORLD_SIZE"] = "1"
        return  # No need to initialize process group
    '''
    if num_gpus == 1:
        ...
        os.environ["RANK"] = "0"
        os.environ["WORLD_SIZE"] = "1"
        return  # No call to dist.init_process_group()

    # MULTI-GPU: Proceed with distributed setup
    if "SLURM_JOB_ID" in os.environ:
        rank = int(os.environ["SLURM_PROCID"])
        world_size = int(os.environ["SLURM_NTASKS"])
        node_list = os.environ["SLURM_NODELIST"]
        ...
        os.environ["MASTER_PORT"] = os.environ.get("MASTER_PORT", str(port + num_gpus))
        os.environ["WORLD_SIZE"] = str(world_size)
        os.environ["LOCAL_RANK"] = str(rank % num_gpus)
        os.environ["RANK"] = str(rank)
    ...
    print(f"[DDP] Initialized process group with rank {rank}, world size {world_size}") # Print the GPU availability
